		...
		for(int rank = 0; rank < 6; rank++)
		{
			for(int i = 0; i < 8; i++)
			{
				for(int j = 0; j < 8; j++)
				{
					...
					if(piece/10 == (color) && piece%10 == (rank+1) && piece != 0)
					{
						printf("the piece is %d, position is %d and %d !!!\n ", piece, i, j);
						printf("Rank is  %d, Color is %d!!!\n ", rank, color);
						vector[shift+color*6*8*8+rank*8*8+i*8+j] = 1;
					}
					else
					{
						vector[shift+color*6*8*8+rank*8*8+i*8+j] = 0;
					}
				}
			}
		}
...
{
	BSTATE test;
	fenToBoardState("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR b KQkq - 0 1", &test);
	int *vectorTest = malloc(sizeof(int)*774);
