...
int main()
{
  ...
  randReluArray((long double *)aw->weight0, 773, 600, 773);
  randReluArray((long double *)dw->weight3, 600, 773, 600);
  int yy, xx;
  for (yy = 0; yy < 773; ++yy)
    {
      for (xx = 0; xx < 600; ++xx)
  ...
  boardToVector(&move, (long double *)vect);
  int t = 0;
  int i;
  for (int i = 0; i < 773; ++i)
    {
